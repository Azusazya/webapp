...
async def api_register_user(*, email, name, passwd):
    ...
    user = User(id=uid, name=name.strip(), admin=1, email=email, passwd=hashlib.sha1(sha1_passwd.encode('utf-8')).hexdigest(), image='http://www.gravatar.com/avatar/%s?d=mm&s=120' % hashlib.md5(email.encode('utf-8')).hexdigest())
    await user.save()
    r = web.Response()
...
async def authenticate(*, email, passwd):
    ...
    r.set_cookie(COOKIE_NAME, user2cookie(user, 86400), max_age=86400, httponly=True)
    user.passwd = '******'
    r.content_type = 'applicatino/json'
    r.body = json.dumps(user, ensure_ascii=False).encode('utf-8')
    return r 
